...
module.exports.getWhishlist = async(req,res) => {
  try{
    ...
    const whishlistDetails = await whishlistCollection.findOne({ userId: userId }).populate('products.productId');
...
module.exports.addWhishlist = async (req, res) => {
  try {
    ...
    if (existingWishlist) {
      ...
      if (existingProduct) {
        return res.status(200).json({ existing: true, message: "Product already in the Wishlist" });
      } else {
