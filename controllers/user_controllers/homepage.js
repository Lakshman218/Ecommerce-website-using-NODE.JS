...
module.exports.getUserRoute = async (req, res) => {
  try {
      ...
      const categoryOffers = await offerCollection.find({ offerType: "category", isActive: true });
      const productOffers = await productCollection.find({ discountStatus: "Active" });
      
      if(req.user){
        const userData = await userCollection.findOne({email:req.user})
        const username = userData.username;
        res.render("userIndex", { loggedIn, username, productdata, categorydata, productOffers });
      } else {
        res.render("userIndex", { loggedIn, productdata, categorydata, productOffers });
      }
